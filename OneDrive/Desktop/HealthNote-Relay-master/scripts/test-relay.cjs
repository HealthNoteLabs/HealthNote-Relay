 ...
 * 
 * Usage: 
 *   node test-relay.cjs <relay-url> [private-key]
 * 
...
if (!privateKey) {
  ...
  console.log(`Generated temporary private key: ${privateKey}`);
}
...
async function testExerciseTemplate() {
  ...
  exerciseTemplate.sig = nostrTools.signEvent(exerciseTemplate, privateKey);
...
async function testWorkoutTemplate() {
  ...
  workoutTemplate.sig = nostrTools.signEvent(workoutTemplate, privateKey);
...
async function testWorkoutRecord() {
  ...
  workoutRecord.sig = nostrTools.signEvent(workoutRecord, privateKey);
...
async function testPrivacyLevels() {
  ...
  publicEvent.sig = nostrTools.signEvent(publicEvent, privateKey);
  
  limitedEvent.id = nostrTools.getEventHash(limitedEvent);
  limitedEvent.sig = nostrTools.signEvent(limitedEvent, privateKey);
  
  privateEvent.id = nostrTools.getEventHash(privateEvent);
  privateEvent.sig = nostrTools.signEvent(privateEvent, privateKey);
